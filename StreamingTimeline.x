...
//  NeoFreeBird
//
//  Native Home-timeline auto-refresh ("streaming"). A floating control sits
...
#import <math.h>
#import <string.h>
#include <execinfo.h>   // backtrace / backtrace_symbols_fd (signal-crash capture)
#include <fcntl.h>
#include <unistd.h>
#include <signal.h>
#include <stdio.h>
#include <pthread.h>    // b68 hang sampler: watcher thread + main-thread mach port
...
static void nfb_columnsAxisRestoreSuppressedScroll(void);
...
// b68: opt-in list for default-hidden timeline entries (For You). The hidden list records
// explicit hides; this records explicit shows — a default-hidden entry is visible only here.
static NSString * const kNFBColumnsShownKey = @"NFBColumnsShownV1";
...
static NSString *nfb_columnTimelineIdentity(UIViewController *vc) {
    ...
    // b68: For You and Following share the items-VC class and can share every identifier part,
    // so For You gets an explicit suffix — the order/hidden prefs must never alias the two.
    // Following keeps its historical identity so saved prefs stay valid.
    if ([NSStringFromClass(vc.class) containsString:@"HomeTimelineItemsViewController"] &&
        nfb_isRecommendedHomeTimeline(vc)) {
        identity = [identity stringByAppendingString:@"|foryou"];
    }
    return identity;
}

static BOOL nfb_shouldUseTimelinePageAsColumn(UIViewController *page) {
    // b68: the For You home page is column-eligible now (user request: a no-auto-refresh
    // "おすすめ" column addable from the column settings). It ships default-hidden via the
    // entry's defaultHidden flag, and every auto-refresh entry point still hard-blocks
    // recommended timelines (nfb_streamTriggerTarget / nfb_streamCanRunForTarget).
    if (!nfb_isTimelinePageController(page)) return NO;
...
// knew about the two home items VCs, so pinned lists never refreshed.
static UIViewController *nfb_selectedTimelineVC(UIViewController *vc) {
    UIViewController *paging = nfb_parentControllerNamed(vc, @"Paging");
    if (paging) {
...
// Crash capture: append uncaught-exception details to the persistent oplog file, so a device
// crash can be read back after relaunch via "Copy saved log" — even when recording was off.
// Installed lazily on the first Home appearance (NOT in a ctor) so the app's own crash
// reporter has already installed its handler; we write our line, then chain to it.
...
static void nfb_uncaughtExceptionHandler(NSException *exception) {
    @try {
        ...
        if (gNFBLogFile) {
            @try { [gNFBLogFile writeData:data]; } @catch (NSException *e) {}
        } else {
            NSFileHandle *fh = [NSFileHandle fileHandleForWritingAtPath:nfb_logFilePath()];
            if (fh) {
                @try { [fh seekToEndOfFile]; [fh writeData:data]; [fh closeFile]; } @catch (NSException *e) {}
            } else {
...
// b67: NSSetUncaughtExceptionHandler only sees Objective-C exceptions. The device crash on
// "search → tweet → open profile" left NO crash block in the saved log — a native fault
// (objc_msgSend to a deallocated object, bad memory, abort) raises a BSD signal instead and
// never reaches the NSException handler. Capture those too. The handler below is restricted
// to async-signal-safe calls (open/write/close + backtrace_symbols_fd on a path and image
// line precomputed at install time), then chains to the previously installed handler — or
// re-raises with the default action — so the system crash flow is unchanged.
...
static void nfb_crashWriteHex(int fd, uintptr_t v) {
...
static void nfb_signalCrashHandler(int sig, siginfo_t *info, void *context) {
    ...
    if (fd >= 0) {
        ...
        nfb_crashWrite(fd, "\nfault addr: ");
        nfb_crashWriteHex(fd, (uintptr_t)(info ? info->si_addr : 0));
        nfb_crashWrite(fd, "\n");
    ...
    // Hand the signal back: the previous handler if one was installed, else default + re-raise.
...
// Normal-context raw append to the persistent oplog file (the same file the crash handlers use).
// Safe from any thread; no-op until the crash logger has resolved the path. Shared with Tweak.x
// (watchdog-suppression notices) via an extern declaration there.
void NFBAppendCrashLogRaw(const char *s) {
    if (!gNFBCrashLogPathC[0] || !s || !s[0]) return;
    int fd = open(gNFBCrashLogPathC, O_WRONLY | O_APPEND | O_CREAT, 0644);
    if (fd < 0) return;
    nfb_crashWrite(fd, s);
    close(fd);
...
// b68 main-thread hang sampler. The b67 signal capture proved the "search → tweet → profile"
// death is Twitter's own hang watchdog calling abort() from a background queue (SIGABRT inside
// TFNTwitterWatchdogLogHang) — so the captured stack shows the killer, never the culprit. This
// watcher samples the MAIN thread's stack while a stall is still in progress: a heartbeat block
// is posted to the main queue twice a second; when it goes stale for >2s the watcher suspends
// the main thread, reads pc/lr and walks the frame-pointer chain with vm_read_overwrite (cannot
// fault, takes no locks — symbolication waits until after thread_resume), and appends up to 3
// samples per episode to the persistent oplog. Survives even if the watchdog is re-enabled,
// and with the b68 watchdog defuse it documents the freezes the user now feels instead of dying.
#if defined(__arm64__)
static mach_port_t gNFBMainThreadPort = MACH_PORT_NULL;
static _Atomic uint64_t gNFBMainBeatMs = 0;
static _Atomic int gNFBMainPingPending = 0;

static uint64_t nfb_uptimeMs(void) {
    return (uint64_t)(clock_gettime_nsec_np(CLOCK_UPTIME_RAW) / 1000000ull);
}

static int nfb_captureMainThreadStack(uintptr_t *frames, int maxFrames) {
    if (gNFBMainThreadPort == MACH_PORT_NULL || maxFrames <= 0) return 0;
    if (thread_suspend(gNFBMainThreadPort) != KERN_SUCCESS) return 0;
    int n = 0;
    arm_thread_state64_t state;
    mach_msg_type_number_t stateCount = ARM_THREAD_STATE64_COUNT;
    if (thread_get_state(gNFBMainThreadPort, ARM_THREAD_STATE64, (thread_state_t)&state, &stateCount) == KERN_SUCCESS) {
        const uintptr_t kPtrMask = 0x0000007fffffffffULL;   // strip PAC/tag bits; user text stays well below 2^39
        uintptr_t pc = (uintptr_t)__darwin_arm_thread_state64_get_pc(state) & kPtrMask;   // b69: strip PAC like lr
        uintptr_t lr = (uintptr_t)__darwin_arm_thread_state64_get_lr(state) & kPtrMask;
        uintptr_t fp = (uintptr_t)__darwin_arm_thread_state64_get_fp(state);
        if (pc) frames[n++] = pc;
        if (lr > 0x4000 && n < maxFrames) frames[n++] = lr;
        uintptr_t prevFp = 0;
        while (n < maxFrames && fp && (fp & 0x7) == 0 && fp > prevFp) {
            struct { uintptr_t fp; uintptr_t lr; } record;
            vm_size_t outSize = 0;
            if (vm_read_overwrite(mach_task_self(), (vm_address_t)fp, sizeof(record),
                                  (vm_address_t)&record, &outSize) != KERN_SUCCESS || outSize != sizeof(record)) break;
            uintptr_t frameLr = record.lr & kPtrMask;
            if (frameLr < 0x4000) break;
            frames[n++] = frameLr;
            prevFp = fp;
            fp = record.fp;
        }
    }
    thread_resume(gNFBMainThreadPort);
    return n;
}

static void nfb_appendHangSample(double stallSeconds, int sampleIndex, const uintptr_t *frames, int frameCount) {
    if (!gNFBCrashLogPathC[0]) return;
    int fd = open(gNFBCrashLogPathC, O_WRONLY | O_APPEND | O_CREAT, 0644);
    if (fd < 0) return;
    char line[512];
    snprintf(line, sizeof(line), "\n=== HANG (main thread stalled %.1fs, sample %d) ===\n", stallSeconds, sampleIndex);
    nfb_crashWrite(fd, line);
    nfb_crashWrite(fd, gNFBCrashImageInfoC);
    for (int i = 0; i < frameCount; i++) {
        ...
        if (dladdr((const void *)frames[i], &info)) {
            if (info.dli_fname) {
                const char *slash = strrchr(info.dli_fname, '/');
                image = slash ? slash + 1 : info.dli_fname;
            }
            if (info.dli_fbase) imageOffset = frames[i] - (uintptr_t)info.dli_fbase;
        }
        if (info.dli_sname && info.dli_saddr) {
            snprintf(line, sizeof(line), "%2d  0x%016lx  %s +0x%lx  %s + %lu\n",
                     i, (unsigned long)frames[i], image, (unsigned long)imageOffset,
                     info.dli_sname, (unsigned long)(frames[i] - (uintptr_t)info.dli_saddr));
        } else {
            snprintf(line, sizeof(line), "%2d  0x%016lx  %s +0x%lx\n",
                     i, (unsigned long)frames[i], image, (unsigned long)imageOffset);
        }
        nfb_crashWrite(fd, line);
    }
    nfb_crashWrite(fd, "=== HANG SAMPLE END ===\n");
    close(fd);
}

static void *nfb_hangSamplerMain(void *arg) {
    (void)arg;
    pthread_setname_np("nfb-hang-sampler");
    uint64_t lastLoopMs = nfb_uptimeMs();
    ...
    for (;;) {
        ...
        if (processWasSuspended) {
            atomic_store(&gNFBMainBeatMs, now);
            atomic_store(&gNFBMainPingPending, 0);
            stallStartMs = 0;
            samplesTaken = 0;
            continue;
        }
        if (!atomic_exchange(&gNFBMainPingPending, 1)) {        // never flood the main queue during a stall
            dispatch_async(dispatch_get_main_queue(), ^{
                atomic_store(&gNFBMainBeatMs, nfb_uptimeMs());
                atomic_store(&gNFBMainPingPending, 0);
            });
        }
        uint64_t beat = atomic_load(&gNFBMainBeatMs);
        if (!beat) continue;
        uint64_t ageMs = now > beat ? now - beat : 0;
        if (ageMs < 2000) {
            if (stallStartMs && samplesTaken) {
                char line[96];
                snprintf(line, sizeof(line), "\n=== HANG RECOVERED after %.1fs ===\n", (now - stallStartMs) / 1000.0);
        ...
        if (samplesTaken < 3 && now >= nextSampleAtMs) {
            uintptr_t frames[64];
            int frameCount = nfb_captureMainThreadStack(frames, 64);
            samplesTaken++;
            nextSampleAtMs = now + 1500;
            if (frameCount > 0) nfb_appendHangSample(ageMs / 1000.0, samplesTaken, frames, frameCount);
        }
    }
    return NULL;
}

static void nfb_startHangSamplerFromMainThread(void) {
    if (![NSThread isMainThread]) return;   // the heartbeat baseline and port must describe the main thread
    gNFBMainThreadPort = pthread_mach_thread_np(pthread_self());
    atomic_store(&gNFBMainBeatMs, nfb_uptimeMs());
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    pthread_t thread;
    pthread_create(&thread, &attr, nfb_hangSamplerMain, NULL);
...
static void nfb_installCrashLoggerOnce(void) {
    ...
    dispatch_once(&once, ^{
        ...
        // Signal-level capture (b67): precompute everything the handler needs — it must not
        // allocate or call into Foundation at crash time.
        ...
        nfb_startHangSamplerFromMainThread();   // b68: main-thread stall stacks into the same log
    });
}
...
static BOOL nfb_streamCanRunForTarget(UIViewController *target) {
    ...
    if (gInlineColumnsEnabled) {
        for (UIViewController *page in nfb_currentColumnRefreshControllers()) {
            ...
            if (nfb_isRecommendedHomeTimeline(page)) continue;   // b68: For You column never auto-refreshes
...
static BOOL nfb_homeTabSelectedOrUnknown(void) {
    ...
    if (page.length) {
        ...
        if (gInlineColumnsEnabled && [page isEqualToString:BHTColumnsHostPageID()]) return YES;
        return NO;
    }
...
static BOOL nfb_streamShouldFire(UIViewController *vc) {
    ...
    if (searchTarget) {
        ...
        NFBLogEvent([NSString stringWithFormat:@"streamShould searchLatest[b70] vc=%@",
...
// (T1TwitterSwift.GuideContainerViewController: search field + the segmented Explore categories).
...
static BOOL gNFBColumnsGuideContentReady = NO;  // latched once the borrowed guide loads content (only surfaced after content-load success; until then the trends fallback stays on top)
...
// resizing (which froze before). Restored on columns-off.
...
static BOOL gNFBExtendedContentActuallyRemoved = NO; // private_removeExtendedContentViewController actually ran → ONLY then may we call add (avoids an unbalanced add)
static UIViewController *gNFBExtRemovedSplit = nil;  // strong: the split we removed the rail from, kept alive until restore so the rail is never abandoned. Cleared (released) on restore. The app-split is long-lived so this is plain ownership, not a cycle.
static BOOL gNFBExtRemoveScheduled = NO;   // a one-shot deferred remove is queued (never removed mid layout pass)
...
static void nfb_collapseColumnsChromeView(UIView *view) {
    ...
    if (objc_getAssociatedObject(view, &kNFBInlineColumnsChromeCollapsedKey)) {
        ...
        // the height -> the intermittent gap band that survived earlier passes.
...
// labels (For You / Following / list names) are CHILD UILabels, so it never matched and never hid.
...
// For-You/Following/list segment bar, whatever its private class is. The height cap stops us from
...
// b67: the "original superview" and "column owner" associations used OBJC_ASSOCIATION_ASSIGN —
// a raw pointer that dangles once the target deallocates; the next read then faults inside
// objc_msgSend with NO NSException (the silent native-crash class seen on device). Box the
// reference weakly instead: readers get nil after deallocation and skip the restore.
@interface NFBWeakBox : NSObject
@property (nonatomic, weak) id object;
@end
@implementation NFBWeakBox
@end
static NFBWeakBox *nfb_weakBox(id object) {
    NFBWeakBox *box = [NFBWeakBox new];
    box.object = object;
    return box;
}
...
static void nfb_rememberColumnOriginalViewState(UIView *view) {
    ...
    objc_setAssociatedObject(view, &kNFBColumnsOriginalSuperviewKey, nfb_weakBox(view.superview), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
static void nfb_restoreColumnOriginalViewState(UIView *view) {
    NFBWeakBox *superviewBox = view ? objc_getAssociatedObject(view, &kNFBColumnsOriginalSuperviewKey) : nil;
    if (!superviewBox) return;
    UIView *superview = superviewBox.object;   // nil when the original superview deallocated
    ...
    view.alpha = alpha ? alpha.doubleValue : 1.0;
    if (clips) view.clipsToBounds = clips.boolValue;
    objc_setAssociatedObject(view, &kNFBColumnsOriginalSuperviewKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
static void nfb_expandColumnsPrimaryWidthIfNeeded(UIScrollView *nativeScrollView) {
    ...
    if (gNFBLogRecording) {
        ...
        if (!logged) {
            logged = YES;
            NFBLogEvent(@"columnsExpand[b70] disabled (prevent split layout crash)");
        }
    }
    return;
    ...
    if (gNFBColumnsExpandStableGapCount >= 4) {
        ...
        if (gNFBLogRecording) {
            NFBLogEvent([NSString stringWithFormat:@"columnsExpand[b70] latch (stuck gap=%.1f, %.1f->%.1f)",
    ...
    if (expanded.count) {
        ...
        if (gNFBLogRecording) {
            NFBLogEvent([NSString stringWithFormat:@"columnsExpand[b70] from=%.1f to=%.1f views=%lu",
                currentWidth, targetWidth, (unsigned long)expanded.count]);
        }
    } else if (gNFBLogRecording) {
        static NSString *lastColumnsExpandMiss = nil;
        NSString *miss = [NSString stringWithFormat:@"columnsExpand[b70] miss from=%.1f to=%.1f chain=%lu",
...
// -tabbedViewController:viewControllerAtIndex: for a SEPARATE instance and only
...
static BOOL nfb_columnsNativeSplitTierGuardBegin(NSString *reason) {
    ...
    if (gNFBLogRecording && reason.length) {
        ...
        NSString *key = [NSString stringWithFormat:@"nativeSplit[b70]: guard %@", reason];
...
static void nfb_columnsApplyNativeSplitTierForPaging(UIViewController *paging, BOOL suppress) {
    ...
    if (!split) {
        ...
        if (gNFBLogRecording) NFBLogEvent(@"nativeSplit[b70]: splitNil");
        return;
    }
    ...
    if (gNFBNativeSplitTierApplying) {
        if (gNFBLogRecording) {
            ...
            NSString *key = [NSString stringWithFormat:@"nativeSplit[b70]: reentry skip %@", suppress ? @"suppress" : @"restore"];
    ...
    @try {
        ...
        if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"nativeSplit[b70]: %@ threw %@", suppress ? @"suppress" : @"restore", e.name ?: @"exception"]);
    }
    ...
    if (gNFBLogRecording) {
        ...
        NSString *key = [NSString stringWithFormat:@"nativeSplit[b70]: %@ split=%@ set=%d update=%d recalc=%d",
...
static void nfb_columnsSetExtendedContentRemoved(UIViewController *paging, BOOL removed) {
    ...
    if (removed) {
        if (UIDevice.currentDevice.userInterfaceIdiom != UIUserInterfaceIdiomPad) {
            if (gNFBLogRecording) NFBLogEvent(@"extContent[b70]: skip notPad"); return;
        }
        if (gNFBExtendedContentRemoved) { if (gNFBLogRecording) NFBLogEvent(@"extContent[b70]: alreadyRemoved"); return; }
        UIViewController *split = nfb_columnsAppSplitForPaging(paging);
        if (!split) { if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"extContent[b70]: splitNil paging=%@", paging ? NSStringFromClass(paging.class) : @"nil"]); return; }
        ...
        if (!nfb_iPadColumnsSearchSidebarVC(paging)) { gNFBExtendedContentRemoved = YES; if (gNFBLogRecording) NFBLogEvent(@"extContent[b70]: noSidebar (nothing to remove)"); return; }
        SEL sel = @selector(private_removeExtendedContentViewController);
        if (![split respondsToSelector:sel]) { gNFBExtendedContentRemoved = YES; if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"extContent[b70]: noSelector on %@", NSStringFromClass(split.class)]); return; }
        if ([defs integerForKey:@"NFBExtContentCrashedBuild"] == kExtBuild) { gNFBExtendedContentRemoved = YES; if (gNFBLogRecording) NFBLogEvent(@"extContent[b70]: skip (crashed before this build)"); return; }
        if ([defs integerForKey:@"NFBExtContentInFlightBuild"] == kExtBuild) {
            ...
            if (gNFBLogRecording) NFBLogEvent(@"extContent[b70]: prior remove crashed; disabled this build");
            return;
        }
        ...
        if (removeOK) {
            ...
            if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"extContent[b70]: removed (columns full-width) split=%@", NSStringFromClass(split.class)]);
        } else {
            if (gNFBLogRecording) NFBLogEvent(@"extContent[b70]: remove threw (not marked removed)");
        }
    } else {
        ...
        if (!gNFBExtendedContentActuallyRemoved) {
            BOOL hadState = gNFBExtendedContentRemoved || gNFBExtRemoveScheduled || gNFBExtRemovedSplit != nil;
            ...
            if (hadState && gNFBLogRecording) NFBLogEvent(@"extContent[b70]: restoreSkipped (never actually removed)");
            return;
        }
        ...
        // live stored split). A detached stored split (window == nil) means it was
        ...
        if (split && [split respondsToSelector:sel]) {
            ...
            [split.viewIfLoaded setNeedsLayout];
            if (gNFBLogRecording) NFBLogEvent(@"extContent[b70]: restored");
        } else if (gNFBLogRecording) {
            NFBLogEvent([NSString stringWithFormat:@"extContent[b70]: restore noop (stale/rebuilt split stored=%@ live=%@)",
...
static void nfb_suppressSplitResidueViews(UIView *root) {
    ...
    if (gNFBLogRecording) {
        ...
        NSString *key = [NSString stringWithFormat:@"splitResidue[b70] hidden=%lu root=%@",
...
static void nfb_columnsRetryRemoveRebuiltExtendedContent(UIViewController *paging) {
    ...
    if (removeOK) {
        ...
        if (gNFBLogRecording) {
            ...
            NSString *key = [NSString stringWithFormat:@"extContent[b70]: reRemoved rebuilt split=%@", NSStringFromClass(split.class)];
...
static void nfb_suppressSecondarySearchHostIfNeeded(UIViewController *paging, UIScrollView *nativeScrollView) {
    ...
    if (gNFBLogRecording) {
        ...
        NSString *key = [NSString stringWithFormat:@"secondaryHost[b70] hidden %@ f=(%.0f,%.0f,%.0f,%.0f)",
...
// top logo bar). Hide it via the nav controller's own API (reclaims the vertical space) ONLY while
...
static void nfb_columnsExpandPrimaryViaConstraint(UIViewController *paging, UIScrollView *nativeScrollView) {
    ...
    if (++gNFBFullWidthApplyCount > 8) {   // split keeps resetting the constant -> stop (never hang)
        ...
        if (gNFBLogRecording) NFBLogEvent(@"fullWidth[b70] latched (constant kept resetting; stopped to avoid hang)");
        return;
    }
    if (gNFBLogRecording) {
        static NSString *lastK = nil;
        NSString *k = [NSString stringWithFormat:@"fullWidth[b70] widen %.1f->%.1f (host=%.1f container=%.1f n=%d)",
...
static void nfb_layoutColumnsOverlayForPaging(UIViewController *paging) {
    ...
    for (NSDictionary *entry in entries) {
        if ([entry[@"kind"] isEqualToString:kNFBColumnEntryKindTimeline]) {
            ...
            if (tabVC) {
                ...
                NFBLogEvent([NSString stringWithFormat:@"appTabColumn[b70] skip id=%@", nfb_columnEntryIdentity(entry)]);
            }
        }
    }
    ...
    for (NSDictionary *entry in layoutEntries) {
        ...
        if (!isTimeline) {
            ...
            if (containsNativePager) {
                ...
                if (gNFBLogRecording) {
                    NFBLogEvent([NSString stringWithFormat:@"appTabColumn[b70] cycleGuard disabled id=%@ vc=%@",
        ...
        if (!isTimeline && appTabScroll && appTabScroll.contentSize.width < 1.0 && appTabScroll.contentSize.height < 1.0) {
            ...
            if (columnView.superview == nativeScrollView) [columnView removeFromSuperview];
            if (gNFBLogRecording) {
                NFBLogEvent([NSString stringWithFormat:@"appTabColumn[b70] zeroContent disabled id=%@ vc=%@", identity, NSStringFromClass(vc.class)]);
            }
            idx++;
            continue;
        }
        ...
        if (isTimeline) {
            ...
            NSString *identity = nfb_columnEntryIdentity(entry) ?: @"-";
            NSString *key = [NSString stringWithFormat:@"appTabColumn[b70] layout id=%@ vc=%@ x=%.0f w=%.0f content=%.0fx%.0f text=%@",
    ...
    if (searchColumnVC && [searchColumnVC isViewLoaded] && searchColumnVC.view) {
        ...
        if (gNFBLogRecording) {
            ...
            NSString *k = [NSString stringWithFormat:@"searchColumn[b70] vc=%@ x=%.0f w=%.0f h=%.0f scroll=%@ content=%.0fx%.0f text=%@",
    ...
    if (gNFBLogRecording) {
        ...
        if (![key isEqualToString:lastLayoutKey]) {
            ...
            NFBLogEvent([NSString stringWithFormat:@"layout[b70] %@ off=%.0f extRemoved=%d", key, nativeScrollView.contentOffset.x, gNFBExtendedContentRemoved ? 1 : 0]);
        }
    }
    nfb_setColumnsSegmentedHiddenForPaging(paging, YES);
}
...
static void nfb_restoreInlineColumns(UIViewController *paging) {
    // Paging-INDEPENDENT restores first (they act on stored globals — the remembered split, the
    // remembered logo nav — so columns-off always undoes them even when the home pager is
    // unreachable):
    ...
    nfb_columnsAxisRestoreSuppressedScroll();   // b70: never leave a column scroll disabled after columns-off
    // b66: bail for FOREIGN pagers before touching any view tree. While columns are off this
    // restore runs for EVERY TFNPagingViewController (the profile tabs pager, search surfaces, …),
    // and the device crash on "search tab → tweet → profile" fired right after these passes ran
...
static void nfb_columnsNoteLayoutPassDuration(CFTimeInterval seconds) {
    ...
    if (gNFBLogRecording && passes > 0) {
        NFBLogEvent([NSString stringWithFormat:@"layoutPerf[b70] passes=%ld avg=%.2fms max=%.2fms busy=%.1f%% window=%.1fs",
...
static NSSet<NSString *> *nfb_columnsShownSet(void) {
    static NSSet<NSString *> *cache = nil;
    static NSInteger cacheGen = 0;
    if (cacheGen != gNFBColumnsPrefsGen) {
        NSArray *a = [NSUserDefaults.standardUserDefaults arrayForKey:kNFBColumnsShownKey];
        cache = [a isKindOfClass:NSArray.class] ? [NSSet setWithArray:a] : [NSSet set];
        cacheGen = gNFBColumnsPrefsGen;
    }
    return cache;
...
static BOOL nfb_columnEntryIsHidden(NSDictionary *entry) {
    ...
    if ([nfb_columnsHiddenSet() containsObject:identity]) return YES;
    // b68: default-hidden timeline entries (For You) stay hidden until explicitly shown.
    if ([entry[@"defaultHidden"] boolValue]) return ![nfb_columnsShownSet() containsObject:identity];
    return NO;
}
...
static void nfb_columnEntrySetHidden(NSDictionary *entry, BOOL hidden) {
    ...
    if ([entry[@"defaultHidden"] boolValue]) {
        // b68: default-hidden entries are opt-in — record explicit shows in the shown list
        // (nfb_columnsSetHidden below keeps the hidden list consistent and bumps the pref gen).
        NSMutableArray *shown = [[NSUserDefaults.standardUserDefaults arrayForKey:kNFBColumnsShownKey] mutableCopy] ?: [NSMutableArray array];
        [shown removeObject:identity];
        if (!hidden) [shown addObject:identity];
        [NSUserDefaults.standardUserDefaults setObject:shown forKey:kNFBColumnsShownKey];
        [NSUserDefaults.standardUserDefaults synchronize];   // b69: do not ride nfb_columnsSetHidden's sync
    }
...
static UIViewController *nfb_columnsAppTabControllerForEntry(NSDictionary *entry, UIViewController *paging) {
    ...
    static NSInteger const kAppTabBuild = 70;
    NSUserDefaults *defs = NSUserDefaults.standardUserDefaults;
    ...
    if (gNFBLogRecording) {
        NFBLogEvent([NSString stringWithFormat:@"appTabColumn[b70] factory OK id=%@ vc=%@", identity, NSStringFromClass(fresh.class)]);
    }
    return fresh;
}
...
static NSString *nfb_columnDisplayName(UIViewController *page) {
    ...
    NSString *cls = NSStringFromClass(page.class);
    if ([cls containsString:@"HomeTimelineItemsViewController"]) {
        return nfb_isRecommendedHomeTimeline(page)
            ? nfb_loc(@"NFB_COL_HOME_FORYOU", @"Home (For You)")
            : nfb_loc(@"NFB_COL_HOME_FOLLOWING", @"Home (Following)");
    }
...
static UIViewController *nfb_firstColumnTimelineAwayFromTopExcept(UIViewController *allowedRevealing) {
    for (UIViewController *page in nfb_currentColumnRefreshControllers()) {
        if (![page isViewLoaded] || page.view.window == nil) continue;
        // b68: the For You column never streams, so it must not keep the new-tweets pill alive.
...
static void nfb_revealAllColumnTops(void) {
    ...
    for (UIViewController *page in refreshControllers) {
        if (![page isViewLoaded] || page.view.window == nil) continue;
        // b69: For You never auto-refreshes — skip explicitly (consistent with trigger/canRun).
        if (nfb_isRecommendedHomeTimeline(page)) continue;
        UIScrollView *sv = nfb_mainScrollViewOf(page);
        if (sv && (sv.isDragging || sv.isTracking || sv.isDecelerating)) continue;
        if (nfb_isTimelineAtTop(page)) nfb_streamTriggerTarget(page);
    }
    NFBLogEvent([NSString stringWithFormat:@"columnsAllTop[b70] top=%lu refresh=%lu",
...
void NFBColumnsRetapFocusAndRefresh(void) {
    ...
    NFBLogEvent([NSString stringWithFormat:@"columnsRetap[b70] focusLeft refresh h=%@",
...
static BOOL nfb_streamTriggerColumns(void) {
    ...
    for (UIViewController *page in pages) {
        ...
        UIScrollView *sv = nfb_mainScrollViewOf(page);
        if (sv && (sv.isDragging || sv.isDecelerating || sv.isTracking)) {
            if (!away && !nfb_isRecommendedHomeTimeline(page)) away = page;   // b68: see pill note above
        ...
        if (nfb_isTimelineAtTop(page)) {
            ...
            did = pageDid || did;
        } else if (!away && !nfb_isRecommendedHomeTimeline(page)) {   // b68: For You never earns the pill
            away = page;
...
static void nfb_columnsAssociateColumnView(UIView *view, UIViewController *owner, NSString *key, NSUInteger index) {
    ...
    if (owner) objc_setAssociatedObject(view, &kNFBColumnsColumnOwnerKey, nfb_weakBox(owner), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
static void nfb_columnsNoteTouchedView(UIView *view, NSString *phase) {
    ...
    if (gNFBLogRecording) {
        NFBLogEvent([NSString stringWithFormat:@"columnTouch[b70] phase=%@ key=%@ idx=%lu view=%@",
...
// b70 axis exclusivity: while a clearly-horizontal columns drag is in flight, the touched
// column's own vertical scroll is disabled so the slide doesn't also scroll the timeline
// (diagonal mush). scrollEnabled is the one lever that suspends touch tracking without
// touching recognizer state (the b50 return-NO ban family). The weak global + restore calls
// at every interaction end point guarantee a column can never stay stuck unscrollable.
static __weak UIScrollView *gNFBColumnsAxisSuppressedScroll = nil;

static void nfb_columnsAxisRestoreSuppressedScroll(void) {
    UIScrollView *sv = gNFBColumnsAxisSuppressedScroll;
    gNFBColumnsAxisSuppressedScroll = nil;
    if (sv && !sv.scrollEnabled) {
        sv.scrollEnabled = YES;
        if (gNFBLogRecording) NFBLogEvent(@"columnsAxis[b70] innerRestored");
    }
}

// First vertically-scrollable descendant = the column's timeline table. Bounded BFS; runs
// at most once per clearly-horizontal gesture begin (never per scroll tick — the b55 lesson).
static UIScrollView *nfb_columnsAxisInnerVerticalScrollOf(UIView *root) {
    if (!root) return nil;
    NSMutableArray<UIView *> *queue = [NSMutableArray arrayWithObject:root];
    NSUInteger hops = 0;
    while (queue.count && hops < 400) {
        UIView *v = queue.firstObject;
        [queue removeObjectAtIndex:0];
        hops++;
        if ([v isKindOfClass:UIScrollView.class]) {
            UIScrollView *sv = (UIScrollView *)v;
            if (sv.contentSize.height > sv.bounds.size.height + 1.0) return sv;
        }
...
static void nfb_columnsAxisSuppressInnerScroll(void) {
    UIView *column = gNFBLastTouchedColumnView;
    if (!column || !column.window) return;
    UIScrollView *inner = nfb_columnsAxisInnerVerticalScrollOf(column);
    if (!inner || !inner.scrollEnabled) return;
    inner.scrollEnabled = NO;
    gNFBColumnsAxisSuppressedScroll = inner;
    if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"columnsAxis[b70] innerSuppressed %@",
...
static BOOL nfb_columnsControllerLooksSystemModal(UIViewController *vc) {
    ...
    return [low hasPrefix:@"ui"] ||
           ...
           [low containsString:@"settings"] ||    // b70: 設定 screens (T1GenericSettingsViewController etc.) were routed into the 340pt column — settings always open full-screen (user report 2026-06-12)
...
static void nfb_columnsDismissDetailNav(UINavigationController *nav) {
    ...
    if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"columnDetail[b70] close key=%@", key ?: @"-"]);
}
...
static void nfb_columnsTrimDetailNavsBeforeOpening(NSString *newKey) {
    while (gNFBColumnsDetailNavControllers.count >= 3) {
        ...
        if (!victim) break;
        if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"columnDetail[b70] evictLRU key=%@", victim]);
...
static void nfb_columnsKickDetailNavContentSoon(UINavigationController *nav) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.6 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        NFBLogEvent([NSString stringWithFormat:@"columnDetail[b70] kickLoad top=%@ leaf=%@ content=%.0f",
...
static void nfb_columnsApplyMobileWebViewInTree(UIView *view, int depth) {
    ...
    if (wkClass && [view isKindOfClass:wkClass]) {
        ...
        @try {
            ...
            NFBLogEvent([NSString stringWithFormat:@"columnDetail[b70] webMobile applied wk=%@", NSStringFromClass(view.class)]);
...
static UINavigationController *nfb_columnsAppTabNavForTouchedColumn(void) {
    ...
    UIViewController *owner = ((NFBWeakBox *)objc_getAssociatedObject(column, &kNFBColumnsColumnOwnerKey)).object;
...
static BOOL nfb_columnsRouteControllerIntoTouchedColumn(UIViewController *vc, NSString *reason, BOOL animated) {
    ...
    if (appTabNav) {
        ...
        gNFBColumnsAppTabRedirecting = NO;
        if (gNFBLogRecording) NFBLogEvent([NSString stringWithFormat:@"columnDetail[b70] appTabRedirect reason=%@ vc=%@ depth=%lu",
    ...
    if (nav) {
        ...
        nfb_columnsScheduleMobileWebCheckForController(vc);
        NFBLogEvent([NSString stringWithFormat:@"columnDetail[b70] push key=%@ reason=%@ vc=%@ depth=%lu",
        ...
        NFBLogEvent([NSString stringWithFormat:@"columnDetail[b70] open key=%@ reason=%@ vc=%@ frame=(%.0f,%.0f,%.0f,%.0f)",
...
// the segmented Explore categories + a long list) we need the *real*
...
static void nfb_appendColumnsDiag(NSMutableString *s, UIViewController *active) {
    ...
    for (NSString *detailKey in gNFBColumnsDetailNavControllers) {
        ...
        [s appendFormat:@"columnDetail[b70] key=%@ top=%@ win=%d hidden=%d alpha=%.2f traitH=%ld navFrame=(%.1f,%.1f,%.1f,%.1f) topFrame=(%.1f,%.1f,%.1f,%.1f) scroll=%@ sframe=(%.1f,%.1f,%.1f,%.1f) content=(%.1f,%.1f)\n",
            detailKey,
    ...
    if (UIDevice.currentDevice.userInterfaceIdiom == UIUserInterfaceIdiomPad) {
        ...
        if (primaryHost) {
            ...
            [s appendFormat:@"nativeSplitDiag[b70] split=%@ suppressed=%d storedLive=%d crashed=%ld\n",
            ...
            [s appendFormat:@"fullWidthDiag[b70] hostW=%.1f containerW=%.1f scrollW=%.1f chainLatched=%d pref=%d\n",
...
static void nfb_layoutActiveHomePaging(void) {
    if (gNFBLayoutActiveHomePagingRunning) {
        if (gNFBLogRecording) {
            ...
            NSString *key = @"layout[b70] activeHome reentry deferred";
    ...
    if (!paging || ![paging isViewLoaded]) {
        if (!gInlineColumnsEnabled) {
            ...
            // Columns are off but we couldn't resolve a pager — still restore the extended-content
...
static void nfb_columnsBeginSizeTransition(void) {
    ...
    if (gNFBLogRecording) NFBLogEvent(@"columnsResize[b70] begin lightLayout");
    nfb_scheduleLayoutActiveHomePagingLight();
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.45 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (!gNFBColumnsSizeTransitioning || fabs(gNFBColumnsSizeTransitionStamp - stamp) > 0.001) return;
        gNFBColumnsSizeTransitioning = NO;
        if (gNFBLogRecording) NFBLogEvent(@"columnsResize[b70] timeout finalLayout");
...
static void nfb_columnsEndSizeTransition(void) {
    ...
    gNFBColumnsSizeTransitioning = NO;
    if (gNFBLogRecording) NFBLogEvent(@"columnsResize[b70] end finalLayout");
    nfb_scheduleLayoutActiveHomePaging();
}
...
static void nfb_scheduleColumnsSegmentedControlHiddenReapply(void) {
    ...
    // re-expand the segment height after the early passes, leaving the intermittent gap.
...
- (void)sendEvent:(UIEvent *)event {
    if (gInlineColumnsEnabled && event.type == UIEventTypeTouches) {
        ...
        // b70: a clearly-horizontal columns drag disables the touched column's vertical scroll.
        // Re-enable the moment the touch sequence finishes — Cancelled included (system gesture
        // takeover), or the column could stay unscrollable.
        for (UITouch *touch in event.allTouches) {
            if (touch.phase == UITouchPhaseEnded || touch.phase == UITouchPhaseCancelled) {
                nfb_columnsAxisRestoreSuppressedScroll();
                break;
            }
        }
    }
...
- (NSInteger)private_splitModeForSize:(CGSize)size displayExtendedContent:(BOOL)displayExtendedContent displaySideBar:(BOOL)displaySideBar {
    if (nfb_columnsShouldForceNativeSplitTierForSplit((UIViewController *)self)) {
        ...
        if (gNFBLogRecording) {
            ...
            NSString *key = [NSString stringWithFormat:@"nativeTier[b70] size=%.0fx%.0f ext=%d->0 side=%d mode=%ld",
...
// NOTE: do NOT stop the timer on disappear. Switching to a pinned list
...
- (void)viewDidAppear:(BOOL)animated { %orig; nfb_installCrashLoggerOnce(); gActiveItemsVC = self; nfb_installButton(self.view.window); nfb_streamStart(self); }
- (void)viewDidDisappear:(BOOL)animated { %orig; nfb_streamStop(self); }
- (void)scrollViewDidScroll:(UIScrollView *)scrollView { %orig; gActiveItemsVC = self; nfb_noteActiveTimelineScroll(scrollView); nfb_visibilityForScroll(scrollView); }
%end

// Pinned-list timelines are hosted by T1URTViewController inside the Home paging
...
// all. directionalLockEnabled does the right thing for free: the pager only scrolls
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (gInlineColumnsEnabled && objc_getAssociatedObject(self, &kNFBInlineColumnsAppliedKey) &&
        ...
        // b70: decide the axis PER GESTURE from TRANSLATION, not velocity. velocityInView at
        // begin time reflects only the first 1-3 touch samples (finger-roll noise) — that's why
        // the b58 velocity test kept judging horizontal swipes as vertical. translationInView at
        // begin time is the net displacement across the whole ~10pt slop UIKit accumulates
        // before firing this callback — a direct integral of the user's intent. Three zones:
        //   ambiguous (small or diagonal) → lock OFF, columns free (never block horizontal — b50);
        //   clearly vertical  (|ty| > 1.8·|tx|) → lock ON: pager stays put, timeline scrolls;
        //   clearly horizontal (|tx| > 1.4·|ty|) → lock OFF + the touched column's own vertical
        //   scroll is disabled for this touch (restored on end/cancel) so the slide is exclusive.
        // A deceleration-stop touch yields translation ≈ (0,0) → ambiguous → nothing locks.
        nfb_columnsAxisRestoreSuppressedScroll();   // stale-suppression safety from any prior touch
        CGPoint t = [(UIPanGestureRecognizer *)gestureRecognizer translationInView:(UIView *)self];
        CGFloat ax = fabs(t.x), ay = fabs(t.y);
        BOOL moved = (ax > 8.0 || ay > 8.0);
        BOOL clearlyVertical   = moved && ay > 1.8 * ax;
        BOOL clearlyHorizontal = moved && ax > 1.4 * ay;
        ((UIScrollView *)self).directionalLockEnabled = clearlyVertical;
        if (clearlyHorizontal) nfb_columnsAxisSuppressInnerScroll();
        if (gNFBLogRecording) {
            NFBLogEvent([NSString stringWithFormat:@"columnsAxis[b70] t=(%.1f,%.1f) %@",
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    nfb_columnsAxisRestoreSuppressedScroll();   // b70 safety net
    if (!decelerate) {
